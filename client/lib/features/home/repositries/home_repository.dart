...
class HomeRepository {
  Future<Either<Failure, String>> uploadSong(
    ...
    try {
      final request = http.MultipartRequest(
          'POST', Uri.parse('${ServerConstants.server_url}/song/upload'));
  ...
  Future<Either<Failure, List<SongModel>>> getAllSongs(
    ...
    try {
      final res = await http
          .get(Uri.parse('${ServerConstants.server_url}/song/list'), headers: {
  ...
  Future<Either<Failure, bool>> favSong(
    ...
    try {
      final res = await http.post(
          Uri.parse('${ServerConstants.server_url}/song/favorite'),
          headers: {
            'Content-type': 'application/json',
            'x-auth-token': token,
          },
  ...
  Future<Either<Failure, List<SongModel>>> getFavSongs(
      {required String token}) async {
    try {
      final res = await http.get(
          Uri.parse('${ServerConstants.server_url}/song/list/favorites'),
          headers: {
            'Content-type': 'application/json',
            'x-auth-token': token,
          });
